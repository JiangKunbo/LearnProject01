package com.klivitam.mypractisedemo.utils;

/**
 * Created by klivitam on 17-8-25.
 */

public class AppConfig {
    public static final String FRAGMENT_TAB = "FRAGMENT_TAB_SEL";
    public static final String FRAGMENT_TAB_CHANGE = "FRAGMENT_TAB_CHANGE";
    public static final String FRAGMENT_ID = "FRAGMENT_ID";
    public static final String FRAGMENT_TYPE = "FRAGMENT_TYPE";
    public static final String NEWS_LINK = "NEWS_LINK";
    public static final String NEWS_TITLE = "NEWS_TITLE";
    public static final String NEWS_POST_ID= "NEWS_POST_ID";
    public static final String NEWS_IMG_RES= "NEWS_IMG_RES";
    public static final String TRANSITION_ANIMATION_NEWS_PHOTOS= "TRANSITION_ANIMATION_NEWS_PHOTOS";
}
